...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.branch','s.contact','s.joindate');
	...
	{
	$condArr[] = "b.id = '".mysqli_real_escape_string($conn,$_GET['speciality'])."'";
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-inr "></i>  Take Fee </button>')
...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','b.branch','s.contact','s.joindate');
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].')"><i class="fa fa-inr "></i> Fee Detail </button>')
